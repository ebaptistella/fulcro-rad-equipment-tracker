...
This repository contains a working Fulcro RAD and Datomic web application which we use for job candidate evaluation. The idea is that we want to ensure that incoming candidates can work with Clojure, Clojurescript, Datomic, and Fulcro.

You might want to start reading [Clojure for The Brave and True](https://www.braveclojure.com/foreword). There are also free online books for Fulcro and RAD, and also many YouTube Videos.
...
1. Do you know how to structurally edit code reasonably well (see instructions below)
2. Did you complete the required tasks?
3. Do you understand what you did in the project? Can you do an additional similar thing while pairing?
4. What are your general habits and understanding?
5. How well do you take direction, and understand/respond to in-person pairing.
...
After you've installed the Cursive plugin, you can import these settings using **File > Manage IDE Settings > Import Settings ...**. You do NOT need to unzip the file. Just point the import at the zip.
...
You need to add a REPL. This is done in IntelliJ **Run > Edit Configurations ...**. You will be adding a Clojure Local REPL.
...
REPL means "Read Eval Print Loop". It's an interactive coding tool. Clojure is a dynamic and interactive language, and code can be changed on the fly.

So, now that we have that working you should play a little bit with Clojure. Open `src/dev/user.clj`. This file is auto-loaded by the REPL on startup (so don't leave garbage in it that can crash your startup!).
...
Now, you can send any or all of this expression to your active REPL to see what happens. If you look at the Cursive Cheat Sheet you want to memorize TWO shortcuts (which may be different than the items shown below):
...
If you've never worked with a LISP variant like Clojure you will find that your editing experience will be dramatically improved by first learning about structural editing. If you try to treat Clojure like more common languages you will find the experience very frustrating.
...
1. **Parinfer:** This mode, which you can enable, automatically tries to infer the proper data structure bracing (e.g. parenthesis placement) based on your indentation of the code. Most people find this pretty useful.
2. **Paredit:** This mode tries to enforce matching braces, which some people love and others hate. I do not recommend this mode for beginners.
3. **Structural Off:** The editor does no automatic structural maintenance.
...
Use an online resource such as [Clojure for the Brave And True](https://www.braveclojure.com/foreword/) to learn some Clojure basics, but use this project and the REPL we just got working to play with examples. While doing those, be sure to practice structural editing.
...
You can do a basic Datomic tutorial to get the basic idea. A good place to start is [Max Datom](https://max-datom.com/).
...
The database for this project is represented by RAD model code in the `src/main/com/example/model` folder. You should read the [Fulcro RAD book](https://book.fulcrologic.com/RAD.html) and understand how things are defined and related by attributes.
...
1. Define new attributes/entities via `defattr` in the model folder.
2. Gather those together into the `src/main/com/example/model.cljc` file
3. (Re)start the server, which will cause it to attempt to generate the schema in Datomic.

The development system uses an in-memory database, and that database should persist as long as you don't kill your REPL.

You are going to create a new schema to track equipment, and to track which accounts in the system have been assigned the use of that equipment.

There will be one entity per piece of equipment, and then assignment entities that track the relation (including historically) of who has the equipment.

### Equipment Tracking Database Schema

| Table       | Column       | Type      | Description                    |
| ----------- | ------------ | --------- | ------------------------------ |
| **equipment** | id           | uuid      | Primary key                    |
|             | kind         | keyword   | One of :laptop, :monitor, :keyboard, :mouse, :furniture (required) |
|             | serial       | string    | Serial number (required)       |
|             | description  | string    | description                    |
| **assignment** | id           | uuid      | Primary key                    |
...
An assignment entity that has no return date is the thing that marks a bit of equipment as "currently assigned". In your code you will need to enforce the rule that equipment cannot be assigned to more than one account at a time.
...
Now that you have schema and data, it's time to be able to see it in the UI. Add UI namespaces and follow the existing code patterns to create reports for equipment and assignments so you can see the content of the database without having to do queries. Reports need a top-level source attribute and a hand-written resolver that can return at least the ID fields of the entities in question. Follow the patterns for the other entities and look for `defresolver` to figure out the best way to do this.

> **NOTE:** You'll need to add the report components to the router and menu UI. See `src/main/com/example/ui.cljs`. It has both of those things.

When dealing with things like `defsc`, routing, and DOM, you will be using Fulcro library constructs. See [Fulcro Developers Guide](https://book.fulcrologic.com) for details about these.

### Notes on Resolvers

Pathom can resolve whatever you query for in the database if it has the type and UUID of the entity. This means that most top-level resolvers that you write will return something like:
...
- Add a resolver and column to the account report that indicates how many bits of equipment are assigned to each account. Extra credit: clicking on the number shows the equipment report pre-filtered by that account.
