...
(comment
  ...
  ;; NOTE: The following ONLY work if the server is running (e.g. start worked above)

  ;; Show the schema of the attributes in the current database
  (let [c  (get datomic-connections :main)
        db (d/db c)]
    (d/q
     '[:find (pull ?e [*])
       :where
  ...
  ;; List the account names with their low-level entity ID
  (let [c  (get datomic-connections :main)
        db (d/db c)]
    (d/q
     '[:find ?e ?v
  ...
  ;; Add an account
  (let [c      (get datomic-connections :main)
        result (d/transact c {:tx-data [{:db/id           "new-account"
                                         :account/id      (new-uuid)
